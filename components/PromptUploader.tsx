...
export default function PromptUploader({ onPromptChange, onFilesChange }) {
  ...
  useEffect(() => {
    fetch("/acceptedFiles.json")
      ...
      .catch((err) => console.error("Failed to load accepted file types:", err));
  }, []);
  ...
  const removeFile = (event: React.MouseEvent<HTMLButtonElement>, fileName: string) => {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4">
      ...
      <div className="flex-1 p-4 border border-base-300 bg-base-200 rounded-lg shadow-md">
        <label className="block text-lg font-semibold mb-2">Add Your Prompt</label>
        <textarea
      ...
      >
        <input {...getInputProps()} />
        <p className="text-lg text-gray-500">Drag and drop files here, or click to browse</p>

        {/* Uploaded Files List */}
        {files.length > 0 && (
          <div className="mt-3 max-h-32 overflow-y-auto scrollbar-thin scrollbar-thumb-base-300 bg-base-100 rounded-md p-2">
            {files.map((file) => (
              <div key={file.name} className="flex items-center justify-between bg-base-200 p-2 rounded-md mb-1">
                <span className="text-sm truncate max-w-[75%]">{file.name}</span>
                <button onClick={(event) => removeFile(event, file.name)} className="btn btn-sm btn-ghost text-error">
                  <X size={16} />
                </button>
              </div>
            ))}
